import { ChangeDetectionStrategy, Component, INJECTOR, Inject, Injector, computed, effect, signal, untracked } from '@angular/core';
...
import { AbstractControl, FormControl, FormGroupDirective, FormsModule, NgForm, ReactiveFormsModule, ValidatorFn, Validators } from '@angular/forms';
      ...
      <div class="form-container">
        <section class="form-section">
          ...
          <mat-form-field>
            ...
            @if (maxError()) {
              <mat-error>A number less than or equal to <strong>100</strong></mat-error>
            }

            @if (minError()) {
              <mat-error>A number greater than or equal to <strong>1</strong></mat-error>
            }
        ...
        <section class="form-section">
          ...
          @if (!isValid()) {
            <button mat-flat-button color="secondary" disabled style="width: 15rem;">Enter Number</button>
          } @else {
            <button mat-flat-button color="primary" (click)="randomizeChartOptions()" style="width: 15rem;">Change Chart Data</button>
          }

          <button mat-flat-button color="primary" (click)="emitRandomValue()">Emit Random Number</button>
...
export class LineChartComponent extends BaseChartComponent {
  ...
  numberError = computed(() => {
    ...
    if (this.ignoreAllErrorValidation()) { return false; }

    if (this._emptyThing === 'empty-thing') {
      this._log(`This random number won't happen, ever ever ever...`, this._randomValue(), 'errors');
    }
  ...
  maxError = computed(() => {
    ...
    if (this.requiredError()) { // Ignore if there's no currentValue
      return false;
    }
  ...
  isValid = computed(() => {
    ...
    if (this.ignoreAllErrorValidation()) { return true; }

    // Will update any time ANY of the below signals change.
    return !this.numberError() && !this.maxError() && !this.minError() && !this.requiredError();
  });
  ...
  readonly form = new FormControl(this.currentValue(), [Validators.required, Validators.max(100), Validators.min(1), isNumberValidator]);
  ...
  constructor(@Inject(INJECTOR) private _injector: Injector) {
    ...
    const loggingEffectRef = effect(() => {
      ...
      this._log('Logging effect output for numberError changes:', [ this.numberError(), untracked(this.currentValue) ], 'effect');
    }, {
    ...
    effect(onCleanup => { // OPTIONAL: onCleanup callback to clean up resources inside callback when the effect is destroyed
      ...
      const timer = setTimeout(() => {
        untracked(() => {
          ...
          this._log('\n', undefined, 'effect');
          this._log('Logging effect output for numberError changes:', numberError, 'effect');
        });
      }, 1000);
    ...
    this.form.valueChanges.pipe(takeUntilDestroyed()).subscribe(valueChange => this.currentValue.set(valueChange));

    this._randomValueSubject.pipe(takeUntilDestroyed()).subscribe(value => this._randomValue.set(value));
  }
